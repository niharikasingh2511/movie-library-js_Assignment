...
// ---- INTERNAL FUNCTIONS FOR THE MOVIE LIST VIEW ----
  ...
  <div class="movie animated fadeIn" data-id="${movie.imdbID}">
   ${img}
    <div class="movie__details">
      ...
      <h1 class="movie__name">${movie.Year}</h1>
    </div>
  </div>
  `;
...
// Render movie list results on page, receives an array
// Then calls the renderMovie for each element on the array
export const renderResults = movie => {
  if (movie.result.Response === 'False') {
    const markup = `
      <div class="container__search">
        <h1 class="results animated fadeIn faster">There were no results for movie list"</h1>
  ...
  `;
    ...
    const markup = `
    <div class="container__search">
      <h1 class="results animated fadeIn faster">MOVIE LIST"</h1>
    ...
    movie.result.Search.forEach(movie => renderMovie(movie));
    renderPagination(movie.page, movie.result.totalResults);
  }
};
